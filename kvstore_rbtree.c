...
rbtree_node *rbtree_successor(rbtree *T, rbtree_node *x) {
	rbtree_node *y = x->parent; // 父节点

	if (x->right != T->nil) {
		return rbtree_mini(T, x->right); // 返回最小值的节点
	}
...
// 红黑树左旋，x是节点的左子树，T是树根节点
// 左旋的条件是节点的左子树非空，且左子树的颜色为红色，左旋的原因是插入的节点的父节点为红色
...
// 红黑树右旋，y是节点的右子树，T是树根节点
// 右旋的条件是节点的右子树非空，且右子树的颜色为红色
